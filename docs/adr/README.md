...
- **[0092](0092-contexts.md)**
  - Decision: Contexts — a demand-free node plus a cross-cutting link saying where a thing can be DONE, and a device preference for where you are that narrows the offer and the list; unlabeled fits everywhere and law 1 never reads it
...
- **[0103](0103-the-card-states-no-moving-number.md)**
  - Decision: The offer card states no number that moves on its own — `#nextup-left` ("About 2h 30m left today", the one permitted number) comes off, and the remainder of the day keeps the home it already had on the opt-in header clock. The card forbade exactly this for the line beneath it, the clock is opt-in because "a day is not a countdown" while the card said it to everybody, the two rendered the same arithmetic at once for anyone with the clock on, and the line's own prospective defense needed the item's duration, which renders only in the detail sheet
...
- **[0110](0110-color-is-checked-by-arithmetic.md)**
  - Decision: The color half of the accessibility gate splits in two. Structure — which (fg role, bg role, floor) pairs the UI renders — is measured in a browser once per release, under a sentinel palette that makes the role mapping injective by construction. Values are arithmetic, per palette, with no browser. Adding a palette needs no accessibility run. In numbers: the walk made 1,660 contrast assertions for two palettes, about four minutes of browser each, and 624 inventory rows reduce to thirteen distinct pairs, so a palette is thirteen computations. The sentinel palette also finds what nothing else could — a hard-coded color is simply not a sentinel — and the first extraction found thirteen form controls the user agent paints rather than the palette, now declared in .color-ua-owned and held both directions, plus a shipped bug where applyTheme never set color-scheme so choosing light on a dark device left every form control white-on-gray. It removes repetition, not the need to walk the app
  - Status: Accepted · changes what the accessibility gate means for color

- **[0111](0111-the-shell-arrives-wearing-the-choice.md)**
  - Decision: The service worker reads the reader's palette and mode out of IndexedDB and writes them onto `<html>` in the bytes it serves, so both are right on the first painted pixel; the app's own apply functions still own every later change. The cold-start flash had been recorded as unfixable without storing the choice somewhere this app deliberately does not, which is true of localStorage and false in general — a worker can read IndexedDB, and this app already had one serving the shell. Measuring it found two flashes rather than one: the MODE flashed too, on a device whose setting disagrees with the choice, and day-to-night is the larger event. Rejected: a cover painted over the app until the read lands, which trades a hue settle for a blank screen and adds a way to stay blank; and the choice in start_url or a per-palette manifest, which goes stale the moment somebody changes their mind after installing. The store is READ and never created, because opening a database that does not exist creates an empty one and would leave a first-ever visitor without the stores Dexie expects
  - Status: Accepted · removes the cold-start flash of palette and mode

- **[0112](0112-the-chain-walks-both-ways.md)**
  - Decision: The line that says where a node sits — *"Part of ⟨title⟩."* — becomes a control that opens that thing's own sheet, on the same mechanism the child rows have used since 1.6.0. The sheet traveled downward and not upward, so somebody who made a container by filing a task under it was told the container's name and handed no route to it; reported from a device in those terms. A door ONLY in the live-parent case, because `placeWords`' three other readings have nowhere to go and a control that opens nothing is the same defect wearing the other face. Law 4 does not forbid it: NOTES records that law's subject as ALTITUDE and records it being misread as a rule about navigation and then cited to close a routing defect. Also: the tree now says what kind each row is, from the `KIND_WORDS` the held card, the detail sheet and both pickers already use — the app teaches the word `project` when you make one and when you file under one, and the one surface listing them never said it again. Rejected: a Projects surface, because `horizons.ts` already refuses project rows on reasoning that holds — a page listing every project is the tree by another name
  - Status: Accepted · the place line is a door, and the tree names its kinds

- **[0113](0113-the-pile-is-counted-the-person-is-not.md)**
  - Decision: The sorting room states how many are in the pile, standing and painted on every repaint, and only once there is a card in front of you — `hidden` in the door state on the same condition as `#triage-card`. Amends ADR-0044's blanket "no numbers while sorting" by separating two instruments that record never told apart: PROGRESS ARITHMETIC measures the person ("19 of 240", "5 left", a percentage, a bar), invites a verdict and turns a pile into a course you are behind on; a COUNT OF WHAT IS PRESENT measures the pile, has no denominator to be a fraction of, and falls because the shelf empties. Everything ADR-0044 lists stays banned. Prompted by one sentence better than the reasoning it overturned — how would I know how many are left after getting distracted and looking back at the screen — because a number stated once at entry, which is what sort mode does, serves a reader who never looks away, and that is not this reader. Its own element, so ADR-0085's gauge keeps its sentence, its exact-string assertion and its no-digit prohibition unchanged. Sort mode is left inconsistent on purpose and the reason is recorded there
  - Status: Accepted · the pile is counted; the person is not

- **[0114](0114-an-arrival-is-a-set-you-can-work-through.md)**
  - Decision: Every node an importer creates carries an `arrival` key — the importing commit's own timestamp, which is unique per run and is also the date the set is labeled with, so nothing is minted. The sort picker offers one door per arrival, generated per key exactly as `under:` and `menu:` already are. Named by date and size and never by a source, because nothing records which planner a file came from — format is sniffed from the content and the filename is never stored, so a name would be a guess presented as a fact. The built-in sample set is an arrival too, under a fixed key, named rather than dated: unnamed it leaves loose rows somebody cannot tell from their own forgotten work in a store they are then afraid to clean. Fixes the same defect 2.38.0 found one turn down from where it looked — `looseFromImport` keyed on `arrived`, which marks only a row that came in with nothing to go on, so a dated row and every project in the same file were missing from a batch named for the whole import. Additive: an older log folds to null and behaves as it did
  - Status: Accepted · an arrival is a set you can work through, and it has a name

- **[0115](0115-a-line-is-somewhere-you-can-stand.md)**
  - Decision: A role's own sheet holds `lineView` — the live work carrying that identity, and the distinct horizons that work sits under, which is the cross-cutting line rendered literally. Closes the app's one structural asymmetry: every link was traversed FORWARD only, from a task outward (`rolesOf`, `servesNode`, `dependencyView`), and a repo-wide search for `roles.includes` returned one hit, the fold's own dedupe — so the attention readout could say attention went to an identity with no path to what was there, and on an item a role's name was a REMOVE control. NOT the filter `roles.ts` refuses, which stands: this is an inspection mode, never a workspace (horizon-models §3), and no working surface narrows. Sorted by name never size, finished work excluded, and an empty line says so because that is the most useful answer it gives. The readout rows and a role's name on an item both become doors. Still unbuilt and named: `review.ts` walks the tree only, so a line with nothing moving on it does not come to you
  - Status: Accepted · a line is somewhere you can stand

- **[0116](0116-complexity-arrives-with-the-structure-that-needs-it.md)**
  - Decision: Disclosure is by emergence, never by grade — a surface exists when it holds something, which is already the house rule (the place chooser hidden until a place is named, the roles readout silent until an identity exists, `serves` printing nothing above work under no horizon, a hub door existing only while something is behind it). Somebody who captures and does sees a box and one thing; somebody running lines, horizons, people and meetings sees those because they made them, and nobody is told which they are. Refuses an expert mode or any capability-gating setting: it is a standing artefact to maintain, it makes two apps to keep consistent, and it grades people — the moment a control is labeled advanced, everybody who has not turned it on knows what that makes them. Names emergence's characteristic defect, which accounts for seven findings in one week: a surface that appears only once the structure exists announces nothing about the structure being possible. So every surface is deliberately QUIET (furniture with nothing to control stays away) or EXPLAINING (a destination somebody opened says what fills it rather than rendering blank), and a new one declares which
  - Status: Accepted · complexity arrives with the structure that needs it

- **[0117](0117-staleness-comes-to-you-or-it-is-not-a-finding.md)**
  - Decision: Review gains a fifth exception, `quietLines` — a role that has carried work and carries nothing live now — and it is the FIRST class in that file that is not a fact about the tree. Every class before it walks `n.parent`, and `heldNodes` excludes roles outright (a role is who work is for, not work), so no exception has ever had a line in hand to examine and a line with nothing running on it was visible only to somebody who opened it. That is the ritual on the person, which ADR-0038 exists to refuse. Three properties carry it, each a refusal of a simpler wrong version: a role that has NEVER carried anything stays out, because staleness needs a before and naming six roles on a fresh store must not report six problems; the walk is TRANSITIVE, because a role is normally carried by a project and asking only what is directly tagged would call every properly-structured line quiet — the mistake `unfedGoals` records having already made once; and it ranks below unfed goals and above quiet areas, sharing a band with the goal because both are a stated direction with no work behind it. No count, no duration, no ordering by staleness, no grade, no color. Closes ADR-0115's own *Still unbuilt* section. Honest cost: the cap keeps it off the demonstration store's screen, so no browser walk renders the class and the assertion is a unit test, stated at the assertion itself
  - Status: Accepted · staleness comes to you, or it is not a finding

- **[0118](0118-a-situation-can-name-who-is-in-the-room.md)**
  - Decision: `situation.saved` carries `people`, a LIST, and the fold normalises it — deduped, sorted, non-strings dropped. Where you are and how long you have were two answers to one question and a saved situation could hold both since 2.21.0; a meeting is a place, a length and a SET OF FACES, and the third could not be stored, which `nd-collisions` entry 24 already recorded and 2.26.0 left a seam for in `setSituation`'s own comment. A list rather than a person is the whole reason this needed a schema change: `where.now` and `with.now` are single-valued device preferences, a meeting has several, and narrowing a working surface to one of them answers a different question — so what will read the whole set is an inspection mode, per ADR-0115 and horizon-models §3. Saving over a name replaces the people WHOLE, or removal is impossible. A person who was let go stops being named with no migration, resolved through `personName` rather than a lookup written beside it. This release writes at most one — whoever the existing filter holds — and the picker for several arrives with the surface that reads several, per ADR-0116's emergence rule. 2.21.0's refusals stand: no count of uses, no last-used, no ordering by frequency, and the closed-shape test that enforces them is what caught the widening
  - Status: Accepted · a situation can name who is in the room

- **[0119](0119-a-meeting-is-somewhere-you-can-stand.md)**
  - Decision: `meetingView` — a situation that names people, opened, showing what is outstanding with each of them and the distinct horizons and lines that work sits under. ADR-0115's move on the other axis, consuming ADR-0118. AN INSPECTION MODE AND NOT A FILTER, the refusal horizon-models §3 and ADR-0115 both already state: nothing narrows, nothing writes `with.now`, and the single-valued person filter stays as it is answering a different question — who is in front of me now, rather than what is this meeting about. Every relation counts, because in a meeting all six are things you would raise. Grouped by person with a joint item under both, per `promisedToAnyone`'s settled case, while the TOTAL is over distinct things so the room is not overstated. An attendee with nothing is kept and says so. Live work only; what was done is `delta.ts`. The picker arrives now because something finally reads several (ADR-0116), as toggles rather than a multi-select, seeded from the filter until touched, with its door on a second line because the row is already three across at the 44px floor. Names what it leaves unmeasured — the two group labels, which render only for a room whose work has a horizon or a role — and names the larger gap above itself: every noun here describes the reader's relationship to work, and none describes a thing in the world with a state of its own and rules about when it may change
  - Status: Accepted · a meeting is somewhere you can stand

- **[0120](0120-the-total-is-the-way-to-the-rest.md)**
  - Decision: `#review-count` becomes a button opening the whole list. The cap stays at three and the surface is unchanged — what changes is that the sentence saying there are more is the way to them. Review's three are taken off the top of a RANKED list of five classes and nothing reached the rest, so the lowest-ranked class was not later but UNREACHABLE, and 3.14.0's quiet lines is fourth of five: on any store with three structural breaks, the class built to make staleness come to you never arrived. Hub LESSONS 187 names the shape (a cap applied after a merge, dropping the row the feature exists to produce) and says the fix is not a bigger cap; this is not one. A door only while there is something behind it, disabled when the three shown are all of them, per ADR-0116. One row builder and one projection paint both lists, because a second of either is how they come to disagree — about what a finding looks like, or about the clock `quietAreas` reads. Refuses grouping by class, a count per class, a bigger cap, re-ranking quiet lines above stalled containers (which outrank on merit), and one-row-per-class (which reaches the fourth class only by raising the cap to five)
  - Status: Accepted · the total is the way to the rest

- **[0121](0121-an-arrangement-can-say-where-it-stands.md)**
  - Decision: two fields on `arrangement` — where it stands, and what would change it — both riding `node.field.set`, so the closed vocabulary does not open. The app had the right noun already and it was one field short: `arrangement` is BINARY, still running or worth confirming, which is the whole answer for a standing order and none of it for a post that cannot be advertised until it is released, a promotion expiring on a date, or a position out to advert. Free text, never an enumeration — the app cannot know the reader's organization and law 7 keeps it out of adjudicating one. The SECOND field earns the feature: a change in the world has no cue to walk past, so the only route to it is monitoring, which is attentionally demanding and rationed by context, and a condition that could change on any day supplies no context to ration by (entry 30, Strong for the cost, Contested for its decomposition). A date that changes it is a clock. Clearing is a WRITE, an empty string, as unmarking writes `false` — the log keeps the decision and the readers treat empty or non-string as nothing. Also gives `arrangementCards` its first caller: written, exported and unit-tested since arrangements landed with NO consumer anywhere, which is hub LESSONS 182's shape. Refuses a defined lifecycle, progress across states, any inference that a condition has been met, a prompt to update on a schedule, and any Done on the list
  - Status: Accepted · an arrangement can say where it stands

- **[0122](0122-who-holds-the-rest.md)**
  - Decision: a pair of relations — `rest-with-them` (*they hold the rest of this*) and `rest-with-me` (*I hold the rest of this*) — plus `holding.released`, the third subtraction, scoped one person and one relation with the relation in the payload because the axis has two directions and one act of release; and the import carrying all three person directions (`@owes` with the sheet's own waiting window, `@promised`, `@holds`), deduped against the store case-insensitively, consumed before the place logic. Q-15 answered by the owner on 2026-09-01; entry 32's only surviving shape, built exactly: a pointer with no text, no version and no age, so there is nothing to conform to, nothing to compare and nothing to order. A PAIR because a one-way noun would encode a deficit (law 7 by shape). No new kind, no new surface, no projection — the pair renders through the sheet, `personView.involves` (days hard-coded null), `namedOn`/`fitsWith` and the meeting room. Refuses every entry-32 prohibition restated: no second account, no reconciliation queue, no aging or tally, no channel. Ships as entry 25's kind of reversible bet, with the Wegner caution as the named watch
  - Status: Accepted · who holds the rest

- **[0123](0123-the-room-by-thing-and-people-with-places.md)**
  - Decision: three answers from one device pass, each inside a standing rule rather than around it. The room gains a second lens — `meetingByThing`, each thing once wearing the names of the attendees on it, the names being the grouping and the whole of the label (no set is ever christened; the reporter's own constraint and law 7's) — while ADR-0119's by-person default stands untouched, and the tests hold the two lenses to the same `total` so they cannot disagree about the room. People carry places (the group that always rendered on a person's sheet now speaks person words there) and `peopleForPlace` shelves every chooser three ways when a place is chosen: stated-here first, the unplaced always offered (`fitsHere`'s load-bearing default restated for people), stated-elsewhere behind one press — stated facts only, never inference (entry 23), a trashed place neither holds nor exiles (resolve-through-state). And the situation gets a second door on the hub in the gauge idiom, the original staying with the consequence lines it narrows. Refuses invented set names, co-occurrence inference, and hiding anyone for good
  - Status: Accepted · the room by thing, and people with places

- **[0124](0124-the-days-ahead.md)**
  - Decision: dated work is visible inside the app, organized by day, current as of now — the calendar story's live half, demanded from use because answer-owed dates change and the `.ics` is a snapshot by design. One selection, three readers: `calendarEntries`/`calendarDay` (`src/ics.ts`) become the single definition of what is dated and on which morning, read by the file, the ⓘ count and the new `datedDays` view (`src/dated.ts`) — nothing may re-derive the walk, which kills the 0.9.0 drift class at the root. Days group in the reader's zone at the diary's own midnight boundary; a passed clock arrives clamped to today exactly as its exported event does, wearing the replan surface's words; no row restates its day (the heading is the one place it is said — ADR-0032). The answer-owed date is named as one, with the waiting-on name beside it, and wins ties at one instant by held's own deadline rule. The export stays what ADR-0007 built: the notification bridge. Replaces an invented refusal — no record ever decided against this view. Refuses a fourth clock walk, `review` on the surface, any past day, and any urgency grading
  - Status: Accepted · the days ahead

- **[0125](0125-the-proof-of-judgment.md)**
  - Decision: a second proof beside the coverage gauge — law 4's analogue, answering *is what I am being shown right* the way law 1 already answered *is anything lost*. `judgmentProof` (`src/assurance.ts`) states where everything is: a fact line under the gauge and a door onto the thing it claims. The places ARE `heldGroups`' groups in its words and order (the rule `whyCovered` follows about `isSilent`'s clauses, one law over), so the proof and the list cannot describe one item differently; the counts are TOTAL over `heldWork` and `holds` can be false, with words that call that the app's fault; and it NAMES what it cannot account for — the review exceptions, computed in `review.ts` and reaching no other surface, each a door. Built because NOTES called it the highest-value thing to build and gated it on Q-11, which closed on the ranking half on 2026-08-17 leaving the trust half standing. Refuses every rank, score, grade and importance order (entry 5, laws 5 and 7), any claim that the state is good, and any second ordering of the places
  - Status: Accepted · the proof of judgment

- **[0126](0126-one-sentence-one-region.md)**
  - Decision: ADR-0105's rule with *sentence* in place of *control* — a confirmation does not live inside anything the act it confirms can remove. One sentence, exactly one live region: the surface's own when the act leaves it standing, `#status` through `sayLasting` (`src/ui/announce.ts`) when the act takes it away. Written because two surfaces wrote a confirmation and removed its home in the same turn — resolving every passed date empties the replan section, taking the amnesty dismisses the greeting — so the sentence reached nobody, and the obvious answer of writing BOTH regions bought a second defect at seven sites, since `#status` is `aria-live` too and a screen reader heard each one twice. The second write's own justification had already expired: it existed because six of those regions were `visually-hidden`, and 3.24.5 made them visible without retiring the reasoning. `tools/announce-check.mjs` asserts the rule over a DERIVED population — a file that writes text and addresses a live region may not name `#status` — so a surface built later is held the day it is written. Settles nothing about whether a sentence ARRIVES, which no static gate can see
  - Status: Accepted · one sentence, one region
