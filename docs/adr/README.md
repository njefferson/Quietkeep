...
- **[0110](0110-colour-is-checked-by-arithmetic.md)**
  - Decision: The colour half of the accessibility gate splits in two. Structure — which (fg role, bg role, floor) pairs the UI renders — is measured in a browser once per release, under a sentinel palette that makes the role mapping injective by construction. Values are arithmetic, per palette, with no browser. Adding a palette needs no accessibility run. In numbers: the walk made 1,660 contrast assertions for two palettes, about four minutes of browser each, and 624 inventory rows reduce to thirteen distinct pairs, so a palette is thirteen computations. The sentinel palette also finds what nothing else could — a hard-coded colour is simply not a sentinel — and the first extraction found thirteen form controls the user agent paints rather than the palette, now declared in .colour-ua-owned and held both directions, plus a shipped bug where applyTheme never set color-scheme so choosing light on a dark device left every form control white-on-grey. It removes repetition, not the need to walk the app
  - Status: Accepted · changes what the accessibility gate means for colour
