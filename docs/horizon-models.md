...
**Quietkeep compatibility.** Covey is the closest civilian ancestor of Q-13 — **a role is an identity that crosses multiple areas** is *First Things First*'s axis, stated cold, sixty years of planner history later arriving at the same noun. What survives: the role as a first-class cross-cutting link (Q-13's settled shape, riding the feeds relation), a role gathering the containers whose review clocks do the returning — **the clock cannot ride the role itself**: `role` is demand-free and the write boundary refuses a clock on a demand-free kind (law 6, ADR-0014), so what comes back on a rhythm is the goal or the area the role labels, and role balance as a **descriptive** attention readout — where attention went per role, plotted, never graded (law 7; ADR-0013 names this readout explicitly). What dies: the mandatory weekly session (thesis), and any per-role quota or "you have neglected Parent this week" copy (laws 5, 7 — that is a rebuke with a spreadsheet behind it).
...
**Why software never offers it.** It is a five-heading essay. There is no schema to compute on; Salesforce built internal tooling for its own V2MOMs on its own platform and the form never generalized beyond the company that is culturally required to write it.
...
- **The reverse walk — BUILT in 3.12.0 (ADR-0115), and it was the thing missing rather than the links.** Every link this app has was traversed forward only: `rolesOf` says which identities a thing carries, `servesNode` which horizon it serves, `dependencyView` what it feeds. None answered the other direction, and a repo-wide search for `roles.includes` returned one hit — the fold's own dedupe. So the attention readout could say attention went to an identity and nothing could show what was there. A role's own sheet now holds `lineView`: the work on the line, and the distinct horizons that work sits under, which is this document's cross-cutting line rendered literally. An inspection mode, never a workspace (§3), so `roles.ts`'s refusal of a "show me only Parent work" filter is untouched.
- **The cross-cutting line — and the Q-13 resonance, stated plainly.** An Army line of effort and a Q-13 role are the *same shape*: a named line that crosses organizational areas, linking activities to an identity or end state, over a tree that cannot contain it. Two separate rulings in this repo — that LOE-style views belong in a planner, and that roles are cross-area identities — are one design fact seen from two sides. The **feeds relation** is the existing machinery for exactly that shape (Q-13 says so in terms), and **the clock does not ride the line**: a role is demand-free, so what returns on a rhythm is the container the line crosses — a goal or an area, neither of which is demand-free. The mountain still comes down; it comes down by the containers rather than by the line (law 4; the decay primitive, law 5-compliant because it *is* the one primitive).
- **Downward lineage projection** (nested purpose, BSC arrows, commander's intent). Law 4. **BUILT in 2.5.0 (ADR-0095)** — `src/serves.ts` renders "serves ⟨goal⟩" on the runway card and on list rows, taking the highest-altitude ancestor rather than the top of the chain. This entry read as a survivor still to build until 2026-08-30, seven months of releases after it shipped, and `serves.ts`'s own header quotes this line back at it. A survey that lists a built thing as unbuilt sends the next session to build it twice.
