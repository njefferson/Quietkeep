...
- **AND 2.13.0's HALF WAS BUILT WITHOUT THE ASKING THIS ENTRY SPECIFIES**, recorded rather than smoothed over: the entry said to ask the owner after he had used filing rather than to guess, and the offer line was built instead from the design this entry itself states — bounded, capped, names only. The defensible half is that the SHAPE was never the unknown; the entry gives it in one sentence. The unknown was whether it is wanted in use, and that was not asked. **If it is wrong on the device, this entry's caution was right and the line comes out** — it is one projection and one element, and removing it costs nothing else.
