...
- **ROUTING PROPOSAL** — **later**, and re-aimed by the check above: the work is to make the one control's door say that BOTH acts live behind it, not to move a control that is already in the right place. It stays *later* rather than becoming a candidate because the app is at its word budget and the honest fix is a rewording somebody should see in use before it is spent.
...
- **ROUTING PROPOSAL** — **later**, and the Q-11 half of its gate is now DISCHARGED: entry 5's interest-aware offering shipped in 2.7.0 ([ADR-0097](adr/0097-the-offer-reads-interest.md)), so what remains here is behind Q-12 alone — whether an app that records nothing when you decline may ever learn what pulls you. Until then the routing is the existing guarantee, worth stating as such in the app's own words: time on the Menu is never measured, compared, or earned.
